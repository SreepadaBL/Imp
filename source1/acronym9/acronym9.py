...
print("CheckList Rule - 19: Availability of acronyms and definition sub-sections in Acronyms and Definition table in Document.")
...
try:
 if iter.endswith('.doc') or iter.endswith('.docx'): 
  ...
  try:
   ...
   #print(res[0])
   #print(res1[0])
   if (re.match("Acronyms",res[0],re.IGNORECASE) and re.match("Definition",res1[0]))or(re.match("Acronyms",res[0]) and re.match("Definitions",res1[0]),re.IGNORECASE):
    print("'",res[0],"'","and","'",res1[0],"'","sub-sections are present in document")
    print("Status:Pass")
   else:
    print("Status:Fail")
  except:
