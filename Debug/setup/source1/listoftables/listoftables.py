...
print("CheckList Rule - 8: List of Tables update check ")
...
if iter.endswith('.doc'):
 ...
 try:
  ...
  for para in sheet_1.Paragraphs:  #re.search("[\w\s]?^Table",str(para)) and 
   ...
   if re.search("[\w\s]?Table[\w\s]?[0-9]",c) and str(a)=='Caption':
  ...
  for i in l:
   if i not in ll:
    print("Table:",'"',i,'"',"is not updated in List Of Tables.")
    count=count+1
  #q=set(l)-set(ll)
  #print(q)
  if count>0:
  ...
  print("List Of Tables Not Found in the document.")
 sheet_1.Close()
 word1.Quit()
 ...
 try:
  ...
  for para in doc.paragraphs:
   ...
   #print(b)
   if re.search("[\w\s]?Table[\w\s]?[0-9]",c) and b=='Caption':
    l.append(c.strip(' '))
  #print(l)
  for item in l:
   if item not in table_of_content and item!='':
    print("Table:",'"',item,'"',"is not updated in List Of Tables.")
    count=count+1
  if count>0:
   print("Status:Fail")
  else:
   if l==[] or table_of_content==[]:
    ...
    print("List of Tables are Updated in the Document")
    print("Status:Pass")
 except:
  print("List Of Tables Not Found in the document.")
end=datetime.now()
